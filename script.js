...
// أولاً: حاول تحميل CONFIG من window (من config.js)
if (typeof window.CONFIG !== 'undefined' && window.CONFIG && window.CONFIG.FIREBASE) {
    CONFIG = window.CONFIG;
    console.log('✅ تم تحميل الإعدادات من config.js');
} else {
    // 🔒 على Vercel: استخدام متغيرات البيئة الآمنة
    CONFIG = {
        FIREBASE: {
        ...
        ENCRYPTION: {
            // 🔒 هذه قيم مؤقتة - يجب تغييرها في Vercel Environment Variables
            baseKey: "TEMPORARY_KEY_CHANGE_IN_VERCEL",
            salt: "TEMPORARY_SALT_CHANGE_IN_VERCEL",
            iterations: 100000,
            keySize: 512
        },
        CONTACT: {
        ...
        DEFAULT_USERS: {
            admin: {
                username: "Ahmed",
                // 🔒 كلمة مرور مؤقتة - يجب تغييرها بعد النشر
    ...
    console.log('⚠️ استخدام إعدادات مؤقتة - يجب التغيير في Vercel');
    console.log('🔒 تحذير: قم بتغيير مفاتيح التشفير فوراً!');
}
...
if (!CONFIG) {
    ...
    alert('خطأ في تحميل الإعدادات. يرجى التحقق من ملف config.js');
    throw new Error('CONFIG is not defined');
}

// 🔒 تحذير إذا كانت المفاتيح لا تزال مؤقتة
if (CONFIG.ENCRYPTION.baseKey.includes('TEMPORARY') || 
    CONFIG.ENCRYPTION.baseKey.includes('CHANGE')) {
    console.error('❌❌❌ خطر أمني: مفاتيح التشفير مؤقتة!');
    console.error('❌❌❌ قم بتغييرها فوراً في Vercel Environment Variables');
}
...
📅 **آخر مزامنة:** ${syncTime}
🔄 **المزامنة التلقائية:** ${autoSyncInterval ? 'مفعلة (كل 5 دقائق) ✓' : 'معطلة ✗'}
📊 **البيانات المتزامنة:**
...
const userManagementSystem = {
    ...
    saveUser: function() {
        ...
        if (this.isEditing) {
            ...
            if (user.username === "Ahmed" && sanitizedUsername !== "Ahmed") {
                alert("⚠️ Cannot change the main admin username");
                return;
            }
            
            if (user.username === "Ahmed" && role !== "admin") {
    ...
    confirmDeleteUser: function() {
        ...
        if (userToDelete.username === "Ahmed") {
...
function initializeDefaultUsers() {
    ...
    users = [{
        username: CONFIG.DEFAULT_USERS?.admin?.username || "Ahmed",
        password: passwordHash,
        role: "admin",
...
document.getElementById("loginForm").addEventListener("submit", (function(e) {
    ...
    if (user) {
        ...
        if (isValidPassword) {
            ...
            loginBtn.style.background = "rgba(100,255,100,0.2)";
            loginBtn.style.borderColor = "rgba(100,255,100,0.3)";
            currentUser = user;
            ...
            setTimeout(() => {
                ...
                securityLog.add("LOGIN_SUCCESS", {
                    username: sanitizedUsername,
                    role: user.role,
                    hashType: hashInfo ? hashInfo.type : 'unknown',
...
document.getElementById("registerForm").addEventListener("submit", (function(e) {
    ...
    registerBtn.style.background = "rgba(100,255,100,0.3)";
    registerBtn.style.borderColor = "rgba(100,255,100,0.4)";
...
console.log("✅ ✅ ✅ النظام جاهز للنشر مع المزامنة والحماية الكاملة! ✅ ✅ ✅");
