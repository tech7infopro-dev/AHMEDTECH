...
class FirebaseManager {
    ...
    updateLocalData(type, remoteData) {
        ...
        try {
            // ✅ الحصول على المستخدمين المحليين الحاليين
            const localUsers = userManager.users || [];
            
            console.log('[Firebase] updateLocalUsers called:', {
            ...
            // أولاً: معالجة المستخدمين القادمين من Firebase
            remoteUsers.forEach(remoteUser => {
                const localUser = localUsers.find(u => u.id == remoteUser.id);
                
                console.log(`[Firebase] Processing user ${remoteUser.id}:`, {
                    remoteHasPasswordHash: !!remoteUser.passwordHash,
                    localFound: !!localUser,
                    localHasPasswordHash: !!localUser?.passwordHash
                });
                
                // ✅ إذا كان المحلي يحتوي على passwordHash والبعيد لا يحتوي → احتفظ بالمحلي كما هو
                if (localUser && localUser.passwordHash && !remoteUser.passwordHash) {
                    console.log(`[Firebase] ✅ Keeping LOCAL user ${remoteUser.id} (has passwordHash)`);
                    processedUsers.push(localUser); // احتفظ بالمحلي كاملاً
                } 
                // ✅ إذا كان البعيد يحتوي على passwordHash → استخدم البعيد (نادر)
                else if (remoteUser.passwordHash) {
                    console.log(`[Firebase] Using REMOTE user ${remoteUser.id} (has passwordHash)`);
                    processedUsers.push({
                        ...remoteUser,
                        id: parseInt(remoteUser.id) || remoteUser.id
                    });
                }
                // ✅ إذا لم يكن هناك passwordHash في أي منهما → استخدم البعيد مع محاولة الحصول على المحلي
                else {
                    console.log(`[Firebase] Using remote user ${remoteUser.id} (no passwordHash anywhere)`);
                    processedUsers.push({
                        ...remoteUser,
                        id: parseInt(remoteUser.id) || remoteUser.id,
                        // محاولة الحصول على البيانات الحساسة من المحلي إذا وجد
                        passwordHash: localUser?.passwordHash,
                        salt: localUser?.salt
                    });
                }
                
                processedIds.add(parseInt(remoteUser.id));
            });

            // ثانياً: إضافة المستخدمين المحليين الذين ليسوا في Firebase (مستخدمين جدد لم يُرسلوا بعد)
            localUsers.forEach(localUser => {
                if (!processedIds.has(parseInt(localUser.id))) {
                    console.log(`[Firebase] ✅ Adding LOCAL-ONLY user ${localUser.id}`);
                    processedUsers.push(localUser);
                }
            });

            console.log('[Firebase] Final processed users:', processedUsers.map(u => ({
                id: u.id,
                username: u.username,
            ...
            if (hasChanges) {
                console.log('[Firebase] Updating users array:', processedUsers.length, 'users');
...
class UserManagement {
    ...
    async login(username, password) {
        ...
        // ✅ تشفير كلمة المرور
        console.log('[Register] Hashing password for:', sanitizedUsername);
        const hashed = await securityManager.hashPassword(password);
        console.log('[Register] Hashed result:', { 
            hasHash: !!hashed.hash, 
            hashLength: hashed.hash?.length,
            hasSalt: !!hashed.salt, 
            saltLength: hashed.salt?.length 
        });
        ...
        console.log('[Register] New user created:', {
            ...
            hasSalt: !!newUser.salt
        });

        this.users.push(newUser);
        
        console.log('[Register] Total users after push:', this.users.length);
        console.log('[Register] Last user in array:', {
            id: this.users[this.users.length - 1].id,
            username: this.users[this.users.length - 1].username,
            hasPasswordHash: !!this.users[this.users.length - 1].passwordHash,
            hasSalt: !!this.users[this.users.length - 1].salt
        });

        await this.saveToStorage();

        console.log('[Register] After saveToStorage - checking localStorage:');
        const stored = securityManager.secureRetrieve(CONFIG.STORAGE_KEYS.USERS);
        const lastStored = stored[stored.length - 1];
        console.log('[Register] Last user in localStorage:', {
            id: lastStored.id,
            username: lastStored.username,
            hasPasswordHash: !!lastStored.passwordHash,
            hasSalt: !!lastStored.salt
        });
    ...
    async saveToStorage() {
        try {
            // ✅ حفظ نسخة كاملة محلياً (مع passwordHash و salt)
            securityManager.secureStore(CONFIG.STORAGE_KEYS.USERS, this.users);
            localStorage.setItem(CONFIG.STORAGE_KEYS.NEXT_USER_ID, this.nextUserId.toString());
            ...
            // ✅ مزامنة مع Firebase (بدون بيانات حساسة)
